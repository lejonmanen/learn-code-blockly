import { serialization } from 'blockly'

// Exportera block till XML (som text)
async function blocklyExport(workspace) {
	const state = serialization.workspaces.save(workspace);
	const jsonText = JSON.stringify(state);
	await navigator.clipboard.writeText(jsonText)
}

// Importera block från XML-fil
async function blocklyImport(workspace) {
	const text = await navigator.clipboard.readText()
	try {
		const state = JSON.parse(text);
		// serialization.workspaces.load(state, workspace)
		updatedLoad(state, workspace)
		return true
	} catch(error) {
		console.log('Fel vid import! Du måste ha tidigare exporterad kod i urklipp.')
		return false
	}
}


// import * as BlocklySerialization from 'blockly/core/serialization';
// const originalLoad = BlocklySerialization.workspaces.load;
const originalLoad = serialization.workspaces.load;
function updatedLoad(state, workspace) {
	// Temporärt ersätta getAllVariables
	const orig = workspace.getAllVariables
	workspace.getAllVariables = function() {
		return workspace.getVariableMap().getAllVariables()
	};

	const result = originalLoad(state, workspace)

	// Återställ originalfunktionen
	workspace.getAllVariables = orig

	return result
}
serialization.workspaces.load = updatedLoad

/*
	// Ladda ner som fil
	const blob = new Blob([xmlText], { type: 'text/xml' });
	const url = URL.createObjectURL(blob);
	const a = document.createElement('a');
	a.href = url;
	a.download = 'blockly_export.xml';
	a.click();
	URL.revokeObjectURL(url);
*/

export { blocklyExport, blocklyImport }
